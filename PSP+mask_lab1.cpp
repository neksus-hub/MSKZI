...
int main()
{
    ...
    cin >> name;
    cout << "Enter a plaintext file name";
    cin >> Name;
    ...
    while (in.read((char*)&data, sizeof(long long int))) {
                for (int i = 0; i < 64; i++) {
                    gamma = gamma << 1;
                    bit_gamma = ((((rlz1 >> 13) ^ (rlz1 >> 12)) & 0x1) ^ (((rlz2[0] >> 39) ^ (rlz2[0] >> 32)) & 0x1));
                    gamma = gamma + bit_gamma;
                    rlz1 = rlz1 << 1;
                    rlz1 = rlz1 + (((rlz1 >> 13) ^ (rlz1 >> 12)) & 0x1);   // Запись бита обратной связи
                    buffer = (rlz2[1] >> 63) & 0x1;
                    rlz2[0] = rlz2[0] << 1;
                    rlz2[1] = rlz2[1] << 1;
                    rlz2[0] = rlz2[0] + buffer;
                    rlz2[1] = rlz2[1] + (((rlz2[0] >> 39) ^ (rlz2[0] >> 32)) & 0x1);   // Запись бита обратной связи
                }
                mask = data ^ gamma;
